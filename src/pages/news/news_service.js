...
import { useGetNewsServiceQuery } from '~/services/new.api';
...
function NewsService() {
  ...
  // Pagination states - separate for desktop and mobile
  ...
  useEffect(() => {
    if (location.pathname === '/tin-tuc/tin-dich-vu') {
      navigate('/tin-tuc/dich-vu', { replace: true });
    }
  }, [location, navigate]);
  ...
  return (
    <div className={cx('news', 'w-full overflow-x-hidden')}>
            ...
            <div className="absolute bg-white shadow-lg rounded-lg z-10 mt-2 md:hidden">
              {menuItems.map((item) => (
                <Link key={item.path} to={item.path} onClick={() => handleMenuItemClick(item)}>
                  <h2
                    className={`${cx('header_menu', { active: location.pathname === item.path })} p-2 hover:bg-gray-200`}
                  >
                    {item.title}
                  </h2>
                </Link>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="space-y-12">
            {newData?.news?.slice(1, 7).map((article) => (
              <Link to={`/tin-tuc/${article._id}`} className={cx('news_link')} key={article._id || article.id}>
                <div className={cx('side_article')}>
      ...
      {/* Service News Section with Pagination - DESKTOP */}
      <div className="hidden md:block">
        <div className={cx('news_wapper', 'overflow-hidden')}>
          <div className="grid grid-cols-3 gap-4">
            {currentDesktopNewsItems.map((item) => (
              <Link to={`/tin-tuc/${item._id}`} key={item._id || item.id}>
                <div className="relative transition-transform duration-300 transform hover:scale-105 hover:shadow-lg p-5">
      ...
      {/* Mobile View - Service News with Pagination */}
      <div className="block md:hidden space-y-6 mt-8">
        ...
        {currentMobileNewsItems.map((article) => (
          <Link to={`/tin-tuc/${article._id}`} key={article._id || article.id} className="block relative mx-2">
            <div className={cx('side_article')}>
              <img src={article.imageUrl} alt={article.title} className="w-1/3 h-32 object-cover" />
              <div className="w-2/3 p-4 flex flex-col">
                <span className="text-lg">{article.category?.name}</span>

...
function NewsKnowlage() {
  ...
  useEffect(() => {
    document.title = 'Y học thường thức || Medical';
    ...
    const fetchAllNews = async () => {
      try {
        ...
        const mainResponse = await fetch(`${process.env.REACT_APP_BACKEND_URL}/news/category/thuong-thuc-y-te?_sort=createdAt:DESC&_limit=1`);
        const mainData = await mainResponse.json();
        setMainNews(mainData);

       // Fetch side news (exactly 6 articles after the first 1)
       const sideResponse = await fetch(`${process.env.REACT_APP_BACKEND_URL}/news/category/thuong-thuc-y-te?_sort=createdAt:DESC&_start=1&_limit=6`);

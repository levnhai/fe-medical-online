...
function Appointment() {
    ...
    pending: (
      <>
        {appointmentData?.pending?.length > 0 ? (
          ...
          <div className="text-center">{t('appointments.noAppointment')}</div>
        )}
      </>
    ),
    paid: (
      <>
        {appointmentData?.paid?.length > 0 ? (
          <div className="text-center">Đã thanh toán</div>
        ) : (
  ...
  return (
    <div className="mx-4 sm:mx-6">
      <h1 className="font-bold text-2xl sm:text-3xl mb-4 sm:mb-6">{t('appointments.title')}</h1>
      <div className="flex flex-wrap justify-center gap-2 sm:gap-4">
  <Button
    className={activeTab === 'paid' ? cx('tab-btn', 'active') : cx('tab-btn')}
    onClick={() => setActiveTab('paid')}
  >
    {t('appointments.tabs.paid')}
  </Button>
  <Button
  ...
  >
    {t('appointments.tabs.canceled')}
  </Button>
</div>

      <div className={cx('tab-content', 'mt-4 sm:mt-6')}>{tabContent[activeTab]}</div>
    </div>
  );
}
